...
.libPaths(c('/lustre/home/ucju659/MyRLibs/',.libPaths()))

library(optparse, lib=.libPaths()[1])
library(ggplot2, lib=.libPaths()[1])
library(bigsnpr, lib=.libPaths()[1])
library(bigreadr, lib=.libPaths()[1])
library(data.table, lib=.libPaths()[1])
library(RhpcBLASctl, lib=.libPaths()[1])
library(future, lib=.libPaths()[1])
...
option_list = list(
  ...
  make_option(c("-m", "--misc"), type="character", 
              ...
              [default= %default]\n
              Note the directory should have a specific structure. See GitHub", metavar="character"),
